      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:pt-40 md:gap-10">
            <div
              ...
              <h3
                className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold text-sky-700/80">
                {item.title}
              </h3>
            </div>
        ...
        <div
          ...
          <motion.div
            ...
            className="absolute inset-x-0 top-0  w-[2px] bg-gradient-to-t from-sky-500 via-sky-800 to-transparent from-[0%] via-[10%] rounded-full" />
        </div>
      </div>
    </div>)
  );
